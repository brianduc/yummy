...
" 2>/dev/null || tofu output
    ...
    echo -e "   AWS_ACCOUNT_ID=\$(aws sts get-caller-identity --query Account --output text)"
    echo -e "   GITHUB_ACTIONS_ROLE_ARN=\$(tofu -chdir=infra/dev output -raw github_actions_role_arn)"
    ...
    echo -e "   GITHUB_REPO=$OPT_REPO"
    echo -e "   AVAILABILITY_ZONES_JSON=[\"${AZ_VALUES[0]}\",\"${AZ_VALUES[1]}\"$(for i in $(seq 2 $((${#AZ_VALUES[@]} - 1)) 2>/dev/null); do printf ',\"%s\"' "${AZ_VALUES[$i]}"; done)]"
    echo ""
