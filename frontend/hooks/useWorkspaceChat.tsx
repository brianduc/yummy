...
import type { ChatMessage, ScanStatus, Session, SystemStatus } from '@/lib/types'
import type { WorkspaceChatContext, TerminalLogEntry } from './useWorkspaceContracts'
...
export interface UseWorkspaceChatOptions {
  ...
  runSdlcStream?: (gen: any) => Promise<boolean>
...
export function useWorkspaceChat(
  ...
  const sendAsk = useCallback(async (question: string, free = false) => {
    ...
    const flushAssistant = (force = false) => {
      ...
      setChatHistory(prev => {
        const next = [...prev]
        if (next[next.length - 1].role === 'assistant') next[next.length - 1] = { role: 'assistant', text: accumulated }
        return next
      })
    }
  ...
  const handleCmd = useCallback(async (rawInput: string) => {
    ...
    try {
      switch (command) {
        ...
        case '/scan': {
          if (!opts.status?.repo) throw new Error('No repo configured. Run /setup first.')
          setBusy(true)
          try { await api.kb.scan() } catch (e: any) { setBusy(false); throw e }
          if (opts.setScanStatus) opts.setScanStatus({ running: true, text: 'Starting scan...', progress: 0 })
          print('🔍 Starting codebase scan...')
          if (opts.setActiveTab) opts.setActiveTab('ide')
        ...
        case '/ask': {
          ...
          if (!opts.status?.kb_has_summary) throw new Error('KB not scanned yet. Run /scan first.')
          await sendAsk(q)
          break
        }
        case '/btw': {
        ...
        case '/cr': {
          ...
          if (opts.setActiveActivity) opts.setActiveActivity('tracing')
          if (opts.fetchMetrics) await opts.fetchMetrics()
          if (opts.setSession) {
            opts.setSession(prev => prev ? {
              ...prev,
          ...
          if (opts.runSdlcStream) {
            const stopped = await opts.runSdlcStream(api.sdlc.startStream(sessionId, req))
        ...
        case '/new': {
          const s = await api.sessions.create() as any
          if (opts.fetchStatus) await opts.fetchStatus()
...
export const WorkspaceChatContextObj = createContext<WorkspaceChatContext | null>(null)
...
export function WorkspaceChatProvider({
  ...
  return <WorkspaceChatContextObj.Provider value={value}>{children}</WorkspaceChatContextObj.Provider>
}

export function useChat() {
  const ctx = useContext(WorkspaceChatContextObj)
  if (!ctx) throw new Error('useChat must be used within WorkspaceChatProvider')
  return ctx
}
