...
function makeHarness(session = baseSession) {
  ...
  const setSession = vi.fn((updater: (prev: Session | null) => Session | null) => {
  ...
  return { ...hook, setSession, print, setBusy, getSession: () => currentSession }
}
...
describe('useWorkspaceSdlc', () => {
  ...
  it('applies stream start, chunk, tool, agent_done, and done events', async () => {
    ...
    let stopped = false
    await act(async () => {
      const promise = result.current.runSdlcStream(events([
        { t: 'start', agent: 'ba' },
        { t: 'c', text: 'Hel' },
        { t: 'tool_call', server: 'srv', tool: 'search', args: { q: 'x' } },
        { t: 'tool_result', server: 'srv', tool: 'search', content: [{ type: 'text', text: 'ok' }], is_error: false },
        { t: 'c', text: 'lo' },
